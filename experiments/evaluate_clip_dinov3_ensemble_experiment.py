...
OUTPUT_DIR = ROOT / "outputs" / "model_experiments" / "clip_dinov3_ensemble"
...
def search_global_alpha(shot_true: np.ndarray, clip_prob: np.ndarray, dino_prob: np.ndarray) -> tuple[dict, pd.DataFrame]:
    rows = []
    best = None
    for alpha in np.round(np.arange(0.0, 1.001, 0.01), 2):
...
def main() -> None:
    ...
    config = TrainConfig("clip_dinov3_focal_ensemble", loss_type="focal")
    ...
    alpha_best, alpha_rows = search_global_alpha(shot_val_true, clip_val_shot_prob, dinov3_val_shot_prob)
    alpha = float(alpha_best["alpha_clip"])
    val_shot_prob = combine_global(clip_val_shot_prob, dinov3_val_shot_prob, alpha)
    test_shot_prob = combine_global(clip_test_shot_prob, dinov3_test_shot_prob, alpha)
    text_threshold, threshold_rows = search_text_threshold(shot_val_true, val_shot_prob, text_val_true, clip_val_text_prob)

    shot_pred = test_shot_prob.argmax(axis=1)
    text_pred = (clip_test_text_prob[:, 1] >= text_threshold).astype(np.int64)
    shot_conf = test_shot_prob.max(axis=1)
    text_conf = np.maximum(clip_test_text_prob[:, 1], 1.0 - clip_test_text_prob[:, 1])
    ...
    pred_df["text_probability_clip"] = clip_test_text_prob[:, 1]
    ...
    per_class_table(text_test_true, text_pred, clip_test_text_prob, ["notext", "text"], "text_class").to_csv(
    ...
    summary = {
        "experiment": "clip_dinov3_focal_ensemble",
        "shot_source": "CLIP ViT-B/32 + DINOv3 ViT-S/16 focal ensemble",
        "text_source": "CLIP ViT-B/32 text head",
        "alpha_clip": alpha,
    ...
    summary.update(classifier_metrics(text_test_true, text_pred, clip_test_text_prob, "text", 2))
